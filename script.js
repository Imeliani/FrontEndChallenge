const url = 'https://api.nasa.gov/planetary/apod?api_key=j0HpTSvPb8oAzIAkrYD8YTeHkOGfoceZpRuO39SP'; //this is the api key to my account
let ii = 0; //this will be the idnex used ot follow the like and unlikes
let firstSubmition = true;

function submition() { //this function will run when the form is submitted, we will take the v alue from the form and get that ,many images from APOD
    ...
    loadMore(value) //run the load function with those pictures
     if (firstSubmition){ //if this is the user's first submission, add a button tha will 
...
function loadMore(index) {
    ...
    $.get(`https://api.nasa.gov/planetary/apod?api_key=j0HpTSvPb8oAzIAkrYD8YTeHkOGfoceZpRuO39SP&count=${index}`, (data, status)=>{ //get request to the nasa apiwith ym key
        console.log(data) //console theb data just to be sure
        for(i=1;i<=index;i++){ //for each images the users wants we will add them to the div grid system
            ...
            <div class="container">
                ...
                <span onclick=" copyURL('${data[i-1].hdurl}', ${ii}) "    ><i class="far fa-share"></i> <span class="share">Copied to Clipboard!</span><span>
